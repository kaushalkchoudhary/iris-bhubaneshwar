...
if [[ "${1:-}" == "--kill-existing" ]]; then
  pkill -f "vite --port ${FRONTEND_PORT}" || true
  pkill -f "go run main.go" || true
  pkill -f "start_all_inference.py" || true
  sleep 1
fi
ensure_databases
resolve_db_urls
...
(
  cd "${ROOT_DIR}/backend"
  nohup env DATABASE_URL="${RESOLVED_DATABASE_URL}" FRS_DATABASE_URL="${RESOLVED_FRS_DATABASE_URL}" GIN_MODE=release GIN_DISABLE_CONSOLE_COLOR=1 GORM_LOG_LEVEL=warn FRS_TOPOLOGY_SYNC_ON_START=1 FRS_TOPOLOGY_CONFIG_PATH="${ROOT_DIR}/backend/config/config.yml" PORT="${BACKEND_PORT}" BIND_ADDR=0.0.0.0 GOCACHE=/tmp/go-build-cache go run main.go > "${DATE_DIR}/go-backend.log" 2>&1 &
...
if [[ "${LOCAL_INFERENCE_ENABLED}" == "1" ]]; then
  (
    ...
    nohup env \
      ...
      ./ANPR-VCC_analytics/.venv/bin/python start_all_inference.py --single-log --combined-log-file "${DATE_DIR}/inference.log" > "${DATE_DIR}/inference-launcher.log" 2>&1 &
    echo "INFERENCE_PID=$!" >> "${PID_FILE}"
  )
fi

cat <<EOF
Started stack with daily logs.
Log folder: ${DATE_DIR}

Files:
- ${DATE_DIR}/frontend.log
- ${DATE_DIR}/go-backend.log
$(if [[ "${LOCAL_INFERENCE_ENABLED}" == "1" ]]; then echo "- ${DATE_DIR}/inference.log        (ANPR API + crowd + FRS + orchestrator in one file)"; fi)
